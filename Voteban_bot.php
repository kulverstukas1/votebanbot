    ...
    private function connectToServer() {
        ...
        if ($this->socket != null) {
            ...
            if ($this->config['registered']) {
                ...
                $this->utils->logToFileAndPrint("[+] Identified");
                // usleep(100);
            }
            ...
            if ($lineParts[0] == 'ping') { // here we check if it is a PING command
                ...
                if ($validVote) {
                    ...
                    switch ($lineParts[3]) {
                        case ':!yes': $yesCount++;
                            if ($this->config['verbose_output']) {
                                $this->utils->logToFileAndPrint("[+] Got yes!");
                            }
                            break;
                        case ':!no' : $noCount++;
                            if ($this->config['verbose_output']) {
                                $this->utils->logToFileAndPrint("[+] Got no!");
                            }
                            break;
                    }
                }
        ...
        while (!$this->linkClosed && !$this->wasKicked) { // yeah, infinite loop... :P
            ...
            // } else if ((count($msg) >= 4) && ($msg[1] != 'notice') || ($msg[2] == $this->config['channel'])) {
                ...
                if ($msg[3] == ':!info') {
                    ...
                    $initProfile['nickname'] = $this->utils->extractNickname($msg[0]);
                    $initProfile['hostname'] = $this->utils->extractHostname($rawOutput);
                    $initProfile['reason'] = $this->utils->extractReason($rawOutput);
                    // print "\n".$initProfile['hostname']."\n";
                    if (($msg[4] != $initProfile['nickname']) && ($initProfile['reason'] != '')) { // another if... this doesn't allow people to ban themselves
                        $canStartVoting = true;
                        ...
                        if ($canStartVoting) {
                            ...
                            if ($banProfile != null) {
                                ...
                                fwrite($this->socket, 'PRIVMSG '.$this->config['channel'].' User '.$msg[4].' does not exist'."\r\n");
                            }
                        }
                    }
